...
class FlowsheetDesign:
    ...
    def get_current_state(self) -> Dict[str, Any]:
        state = {
            ...
            "chosen_open_stream": None, # Optional[Tuple[int, str]]
            ...
            "add_solvent": {
                ...
                "amount_value": None},
            "mixer": None}, #Optional[Dict] = {} # tuple of index, value of the 2nd o/p stream
            "npv_raw": None,
            "npv_norm": None,
        ...
        try:
            ...
            if self.level == 0:      
                if action_index == 0:  # Check this later 
                    ...
                    self.current_state['identifier'] = self.generate_custom_flowsheet_id()
                ...
                if self._chosen_unit_name() == "distillation_column":
                    if action_index > len (self.env_config.DF_distillation_map):
                        ...
                        self.current_state["pending_params"]["distillation_column"] = (action_index, self.DF_distillation)
                        self.history.append(action_index)
                        done, reward, worked = self._complete_action_place_and_simulate()
                        self.current_state["completed_design"] = done 
                        self.objective = reward 
                ...
                # for selecting split ratio 
                    ...
                    if action_index > len(self.env_config.split_ratio_map):
                        ...
                        self.current_state["pending_params"]["split"] = (action_index, self.split_ratio)
                        self.history.append(action_index)
                        done, reward, worked = self._complete_action_place_and_simulate()
                        self.current_state["completed_design"] = done 
                        self.objective = reward 
    ...
    def get_embeddings_from_experts(flowsheets: List['FlowsheetDesign']):
        ...
        batch_latent_nodes_embeds, batch_latent_edges_embeds, batch_latent_open_streams_embeds= [], [], []
        ...
        for fs in flowsheets: 
            ...
            for nid in node_ids:
                ...
                if node_data["unit_type"] == "feed":
                    flow_emb = fs.unit_experts["flow_expert"](node_data)
                    open_emb = fs.open_stream_expert(flow_emb)
                    emb = flow_emb
                ...
                open_stream_embeds.append(open_emb)
            ...
            for u, v, edge_data in fs.sim.graph.edges(data=True):
                ...
                if "unit_type" in edge_data:
                    edge_embeds[ui, vi] = fs.edge_expert(edge=edge_data, edge_exists=True, is_recycle = edge_data.get("is_recycle", False), feed_emb = flow_emb)
                else:
                    edge_embeds[ui, vi] = fs.edge_expert(edge=edge_data, edge_exists=True, is_recycle = edge_data.get("is_recycle"), feed_emb = flow_emb)
            ...
            batch_latent_open_streams_embeds.append(open_stream_embeds)

        return dict(batch_latent_nodes_embeds = batch_latent_nodes_embeds, 
                batch_latent_edges_embeds = batch_latent_edges_embeds, 
                batch_latent_open_streams_embeds = batch_latent_open_streams_embeds,
    ...
    def log_probability_fn(trajectories: List['FlowsheetDesign'], network: nn.Module, device: torch.device, config) -> List[np.array]:
        ...
        with torch.no_grad():
            with torch.amp.autocast(device_type=config.training_device):
                ...
                valid_nodes = batch['node_valid_mask']
    ...
    def list_to_batch(flowsheets: List['FlowsheetDesign'], include_feasibility_masks: bool = False, device: torch.device = None) -> dict:
        ...
        * "open_stream_mask": a boolen tensor denoting whether something is a real open stream
        ...
        batch_nodes, batch_edges, batch_open_streams = [], [], []
        
        # stack nodes within flowsheets 
        for ns, es, os in zip(embeddings_dict["batch_latent_nodes_embeds"], embeddings_dict["batch_latent_edges_embeds"], embeddings_dict['batch_latent_open_streams_embeds']):
            ns = [n.squeeze(0) for n in ns] # make dimension (512, )
            fs_nodes = torch.stack(ns, dim=0)   # (N, d)
            fs_edges = es
            os = [o.squeeze(0) for o in os] 
            fs_os = torch.stack(ns, dim=0)
            batch_nodes.append(fs_nodes)
            batch_edges.append(fs_edges)
            batch_open_streams.append(fs_os)
        ...
        batch_nodes_open_streams_padded = pad_sequence(batch_open_streams, batch_first=True, padding_value=0.0)
        ...
        # levels info
        batch_level_idx = [fs.current_state['current_level'] for fs in flowsheets]

        return_dict = dict(
            batch_latent_nodes = batch_nodes_padded,                  # (B, num_nodes, D))
            batch_latent_edges = batch_edges_padded,
            batch_latent_open_streams = batch_nodes_open_streams_padded,                
            recycler_masks = recycler_masks,        
            mixer_masks = mixer_masks, 
            node_valid_mask = node_valid_mask, 
            levels = torch.tensor(batch_level_idx, dtype=torch.long, device=device), 
        ...
        if include_feasibility_masks:
            ...
            num_actions_per_level_and_flowsheet = [
                ...
                else 0 for fs in flowsheets], #lvl 2 
                ...
                else 0 for fs in flowsheets],  # lvl 3
