...
def build_logit_tensors_per_level(dataset, input_data, terminate_or_open_streams_logits, unit_predictions, indices_for_tracking):
    ...
    num_actions_per_level_and_flowsheet = [
                [len(fs.current_state['open_streams']) + 1 for fs in dataset._flat_sequences],  # lvl 0 
                [fs.env_config.num_units for fs in dataset._flat_sequences],  # lvl 1
                ...
                else 0 for fs in dataset._flat_sequences], #lvl 2 
                ...
                else 0 for fs in dataset._flat_sequences],  # lvl 3
                
            ]
    
    all_lvl_zero_logits = torch.zeros(len(indices_for_tracking), max(num_actions_per_level_and_flowsheet[0]))
    all_lvl_one_logits = torch.zeros(len(indices_for_tracking), max(num_actions_per_level_and_flowsheet[1]))
    all_lvl_two_logits = torch.zeros(len(indices_for_tracking), max(num_actions_per_level_and_flowsheet[2]))
    all_lvl_three_logits = torch.zeros(len(indices_for_tracking), max(num_actions_per_level_and_flowsheet[3]))
    ...
    for i, fs in enumerate(actual_flowsheets_dataset):
        ...
        if fs.level == 2:
            ...
            if unit_name == "mixer":
                ...
                lvl_two_logits = target_scores[input_data['node_valid_mask'][i][1:]]
